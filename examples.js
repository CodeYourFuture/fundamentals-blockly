...
function findExercises() {
  let $exercises = [...document.querySelectorAll(".example")];
...
function addDomElements(exercises) {
  for (let i = 0; i < exercises.length; i++) {
    ...
    if (i > 0) {
      $nav.appendChild(createNavElement("previous example", exercises[i - 1]));
    }
    if (i < exercises.length - 1) {
      $nav.appendChild(createNavElement("next example", exercises[i + 1]));
    }
    ...
    // Start code is the value of "v" in the querystring generated by "Share" button.      
    let start_code = $zeroMd.getAttribute("start_code");
    
    if (start_code) {
      let data = JSON.parse(JSONCrush.uncrush(decodeURIComponent(start_code)));
      exercise.blocklyDomEditor.init(data.h, data.j);
    } else {
